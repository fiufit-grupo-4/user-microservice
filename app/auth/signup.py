...
def signup(credentials: UserBasicCredentials, request: Request):
    ...
    user = User(credentials.mail, hashed_password, role=credentials.role, phone_number=credentials.phone_numer)
    ...
    send_whatsapp_validation_code(credentials.phone_numer)

    return UserResponse(id=str(user_id), mail=user.mail, phone_number=user.phone_number)


@router.post("/validate_verification_code")
async def validate_verification_code(phone_number: str, verification_code: str, request: Request):

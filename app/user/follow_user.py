...
@router.post('/{id_user}/follow', status_code=status.HTTP_200_OK)
async def follow(request: Request, id_user_to_follow: ObjectIdPydantic, id_user: ObjectId = Depends(get_user_id)):
    ...
    return JSONResponse(
        status_code=status.HTTP_400_BAD_REQUEST,
        content=f'User does not exist'
    )


@router.post('/{id_user}/unfollow', status_code=status.HTTP_200_OK)
async def unfollow(request: Request, id_user_to_unfollow: ObjectIdPydantic, id_user: ObjectId = Depends(get_user_id)):
    users = request.app.database["users"]

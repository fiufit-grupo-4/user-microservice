...
from app.settings.auth_settings import Settings, JWT_SECRET, JWT_ALGORITHM
...
def update_user_block_status(user_id: ObjectIdPydantic, request: Request, my_user_id: ObjectId = Depends(get_user_id),
                             block: bool = True, content: str = ""):
...
def block_user(user_id: ObjectIdPydantic, request: Request, my_user_id: ObjectId = Depends(get_user_id)):
    return update_user_block_status(user_id, request, my_user_id, block=True, content="User blocked successfully")


@router.patch("/{user_id}/unblock", status_code=status.HTTP_200_OK)
def unblock_user(user_id: ObjectIdPydantic, request: Request, my_user_id: ObjectId = Depends(get_user_id)):
    return update_user_block_status(user_id, request, my_user_id, block=False, content="User unblocked successfully")
